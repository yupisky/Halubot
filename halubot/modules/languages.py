...
from halubot import dispatcher, spamfilters, LOGGER
from halubot.modules.disable import DisableAbleCommandHandler
from halubot.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from halubot.modules.sql import languages_sql as sql
...
for x in os.listdir('halubot/modules/langs'):
	if os.path.isdir('halubot/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("halubot.modules.langs." + x)
