...
import halubot.modules.sql.welcome_sql as sql
from halubot import dispatcher, OWNER_ID, LOGGER, spamfilters
from halubot.modules.helper_funcs.chat_status import user_admin, is_user_ban_protected
from halubot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from halubot.modules.helper_funcs.msg_types import get_welcome_type
from halubot.modules.helper_funcs.string_handling import markdown_parser, \
	escape_invalid_curly_brackets, extract_time
from halubot.modules.log_channel import loggable

import halubot.modules.sql.feds_sql as fedsql
		...
		if json["result"]["offenses"] > 0:
			...
			if fed_id:
				...
				if not x:
					...
					return
				text = "*New FedBan*\n*Fed:* `TeamNusantaraDevs`\n*FedAdmin*: {}\n*User:* {}\n*User ID:* `{}`\n*Reason:* [CAS ban](https://combot.org/cas/query?u={})".format(mention_markdown(692882995, "Halubot"), mention_markdown(user_id, user.first_name + (" " + user.last_name if user.last_name != None else "")), user_id, user_id)
