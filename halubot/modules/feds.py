...
from halubot import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, TEMPORARY_DATA, LOGGER, spamfilters
from halubot.modules.helper_funcs.handlers import CMD_STARTERS
from halubot.modules.helper_funcs.misc import is_module_loaded, send_to_list
from halubot.modules.helper_funcs.chat_status import is_user_admin
from halubot.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from halubot.modules.helper_funcs.string_handling import markdown_parser
from halubot.modules.disable import DisableAbleCommandHandler

import halubot.modules.sql.feds_sql as sql
...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		elif fednam == "Halubot Official Support":
			fed_id = "HalubotSupport"
	...
	if args:
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				output.name = "halubot_fbanned_users.json"
				update.effective_message.reply_document(document=output, filename="halubot_fbanned_users.json",
			...
			for users in getfban:
				...
				backups += "\n"
			with BytesIO(str.encode(backups)) as output:
				output.name = "halubot_fbanned_users.csv"
				update.effective_message.reply_document(document=output, filename="halubot_fbanned_users.csv",
													caption=tl(update.effective_message, "Total {} pengguna terkena blokir federasi {}.").format(len(getfban), info['fname']))
			return
